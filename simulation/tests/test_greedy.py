...
from solver.matching import greedy_sequential_matching
...
class GreedySolverTestCase(unittest.TestCase):
    def make_matrix(self, m: int = 10, capacity: int = 10, dr: float = 10.0):
        data = pd.read_parquet('_data/sample_snapshot.parquet')
        self.dist_matrix = pd.pivot_table(data, values='distance', index='userid', columns='driver_id').values
        self.assignments = greedy_sequential_matching(self.dist_matrix, m=m, capacity=capacity, dr=dr)
